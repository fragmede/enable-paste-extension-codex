...
- Privacy and 1Password browser overlays appeared over the hosted card modal and captured focus during the live test. After closing the overlay, further field interaction caused the site to navigate to `submitOffer.do`.
- Correction to the initial read: the final page text reported "Thank You For Your Order" with current and deferred charges, so the live flow appears to have completed even though the final offer submit button was not intentionally clicked. Further live testing must not use this transaction page without explicit confirmation at each submit-capable step.
